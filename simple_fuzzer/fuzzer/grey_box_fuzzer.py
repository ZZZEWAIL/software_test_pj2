...
├───────────────────────┼───────────────────────┼───────────────────┼────────────────┼───────────────────┼────────────────┤""")
    ...
    def _reload_offloaded_seeds(self) -> None:
        """将磁盘上所有 offloaded seed 重新加载回内存 population。

        这样调度器可以重新看到这些 seed 并给它们分配能量，
        防止 offload 变成永久删除。重载后再由 _try_offload_population
        按能量高低再次卸载低能量 seed，形成良性循环。
        """
    ...
    def _try_offload_population(self) -> None:
        ...
        在排序前先调用 assign_energy 确保所有 seed（包括刚添加的、
        energy=0 的新 seed）获得最新的能量赋值，避免新发现覆盖的
        有价值 seed 被错误地当作"低能量"卸载。
        """
        if not self._offloaded_seeds_dir:
            return
    ...
    def create_candidate(self) -> str:
        ...
        Periodically reloads offloaded seeds so the scheduler can see them
        again, preventing offload from becoming a one-way black hole.
        """
        # Periodically reload offloaded seeds back into memory
...
├───────────────────────┼───────────────────────┼───────────────────┼────────────────┼───────────────────┼────────────────┤"""
    ...
    def run(self, runner: FunctionCoverageRunner) -> Tuple[Any, str]:  # type: ignore
        ...
        # Use set difference instead of length comparison to correctly
        # detect genuinely new coverage (Bug 3 fix).
        new_coverage = runner.all_coverage - self.covered_line
        if new_coverage:
            self.covered_line |= new_coverage
            # Save seed for ANY new coverage, including when the run crashed.
            # A crashing input that also discovers new code paths is valuable.
